    ...
    <div style="display: grid; grid-template-columns: 280px 1fr; gap: 24px; height: calc(100vh - 150px);">
      ...
      <div class="glass-card" style="display:flex; flex-direction:column; padding: 20px; overflow-y:hidden; gap: 16px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <button id="add-project-btn" class="header-btn" title="Create New Project" style="width:30px; height:30px; border-radius:6px;">
      ...
      <!-- Right side: Kanban board workspace -->
...
async function populateAssigneeFilters() {
  ...
  employees.forEach(emp => {
    ...
    const usernameKey = (emp.contact && emp.contact.includes('@')) ? emp.contact.split('@')[0] : emp.id.toLowerCase();
...
function bindEventListeners(container) {
  ...
  document.getElementById('add-task-btn').addEventListener('click', async () => {
    ...
    const assigneesHTML = employees.map(emp => {
      const uKey = (emp.contact && emp.contact.includes('@')) ? emp.contact.split('@')[0] : emp.id.toLowerCase();
